...
func TestSimpleFlow(t *testing.T) {
	// p(x) = x^3 + x + 5
	p := primitives.NewPoly([]*big.Int{
		big.NewInt(5),
		big.NewInt(1), // x^1
		big.NewInt(0), // x^2
		big.NewInt(1), // x^3
	})
	assert.Equal(t, "x³ + x¹ + 5", primitives.PolynomialToString(p))
...
func TestBatchProof(t *testing.T) {
	...
	assert.Equal(t, "10x⁴ + x³ + x¹ + 5", primitives.PolynomialToString(p))
	...
	z0 := big.NewInt(3)
	y0 := primitives.PolynomialEval(p, z0)

	// 2nd point: p(10)=1015
	z1 := big.NewInt(10)
	y1 := primitives.PolynomialEval(p, z1)

	// 3nd point: p(256)=16777477
	z2 := big.NewInt(256)
	y2 := primitives.PolynomialEval(p, z2)
	...
	ys[2] = primitives.PolynomialEval(p, zs[2])
	v = VerifyBatchProof(ts, c, proof, zs, ys)
	assert.False(t, v)
}
